...
public class ProcessBody {
    ...
    public void addMembers(ArrayList<UserApi> users){
        for (UserApi user: users){
            members.add(0, new Member());
            members.get(0).setName(user.getName());
            members.get(0).setEmail(user.getEmail());
            members.get(0).setDocumentType("CPF");
            members.get(0).setDocumentCode(user.getCPF());
        }
    }
}
